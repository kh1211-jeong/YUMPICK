...
| GA 태그 정상 삽입 | ⚠️ **측정 ID 미설정** | 코드(`lib/analytics.ts`, `app/layout.tsx`)는 완성, `NEXT_PUBLIC_GA_ID`만 채우면 즉시 수집 시작 |
...
### 제출 전 남은 액션
1. **GA4 속성을 만들어 측정 ID(`G-XXXXXXX`)를 Vercel 환경변수에 추가** — 유일하게 비어있는 체크리스트 항목
2. 5분 발표 영상 촬영 (시연 최소 경로 + 6절 실사용 데이터 화면 캡처를 근거 자료로 활용 추천)
